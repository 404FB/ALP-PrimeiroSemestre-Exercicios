...
/*
  ...
  DESCRIÇÃO....: TRADICIONAL JOGO DA VELHA ONDE O OBJETIVO DO JOGO É FAZE UMA SEQUÊNCIA DE "X" OU DE "O". O JOGADOR DEVE INFORMAR O NÚMERO DA LINHA E DA COLUNA ONDE DESEJA FAZER A JOGADA.
*/
main()
{
	...
	do
	{
		...
		for(i = 0;i <= 3; i++)
		{
			for(j = 0; j <= 3; j++)
			{
				matriz[i][j]='\0'; //esvazia os vetores
				verifica[i][j]='\0'; //esvazia os vetores
			}
		}
		do
		{
		...
		else
		{
			...
			verifica[linha][coluna]=1; //coloca o valor 1 em jogadas já feitas
		}
		//VERIFICA LINHAS.
